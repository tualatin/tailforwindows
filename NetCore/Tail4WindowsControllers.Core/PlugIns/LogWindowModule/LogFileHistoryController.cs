...
namespace Org.Vs.Tail4Win.Controllers.PlugIns.LogWindowModule
{
  /// <summary>
  ...
  public class LogFileHistoryController : IHistory<LogFileHistoryData>
  {
    ...
    public async Task<LogFileHistoryData> ReadHistoryAsync(CancellationToken token)
    {
      ...
      try
      {
        return await Task.Run(() => JsonUtils.ReadJsonFile<LogFileHistoryData>(_historyFile), token).ConfigureAwait(false);
      }
      finally
      {
        _semaphore.Release();
